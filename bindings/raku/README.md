...
The feature branch is a build-only `4.0.0-rc.6` candidate. Build the native
...
[`bindings/api.json`](../api.json) is the single machine-readable contract for
the Raku boundary. It records every public lling-llang C function, every Raku
provider-shim function and callback, all ABI-v2 enums and layouts, introduction
revisions, ownership, nullability, threading requirements, and required or
optional capabilities. The generated module contains NativeCall declarations
and the three layouts that Rakudo can represent portably. Identity and digest
layouts containing fixed-size inline C arrays remain pointer-only, and that
limitation is explicit in the model rather than silently approximated.

The generator checks the model independently against
[`include/lling_llang.h`](../../include/lling_llang.h) and
[`cbits/provider.c`](cbits/provider.c) before it emits Raku. Thus a change to
any one of the model, C declarations, callback typedefs, or generated module
fails locally and in CI until all witnesses agree.
...
The self-test is a negative control: it mutates the in-memory API revision and
must prove that the committed generated bytes would become stale. The public
`Lling::Llang` module contains only idiomatic ownership, validation, builder,
graph, algebra, and provider behavior; project NativeCall declarations and
ABI-v2 CStruct layouts must remain in `GeneratedAbi.rakumod`.
...
1. Change the C ABI, provider shim, and `bindings/api.json` together.
2. Run `python3 scripts/generate-raku-abi.py --write`; never hand-edit
   `GeneratedAbi.rakumod`.
