...
The canonical symbols-and-acronyms reference lives at
[`docs/NOTATION.md`](docs/NOTATION.md). It defines every operator ($`\oplus`$,
$`\otimes`$, $`\bar{0}`$, $`\bar{1}`$, $`\circ`$, $`\pi`$, $`\eta`$, $`\infty`$,
$`\varepsilon`$), automata symbol ($`Q`$, $`q_0`$, $`F`$, $`\Sigma`$, $`\Gamma`$,
