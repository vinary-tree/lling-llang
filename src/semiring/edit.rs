...
//! let edits: Vec<_> = combined.sequences().next().unwrap().collect();
...
impl EditOp {
    ...
    pub fn output_char(&self) -> Option<char> {
        match self {
            ...
            EditOp::Transpose { a, b } => Some(*b), // b comes first after transpose
        }
    }
...
mod tests {
    ...
    fn test_sequence_concatenation() {
        ...
        // Check the sequence
        let seq: Vec<_> = chain.sequences().next().unwrap().collect();
        assert_eq!(seq.len(), 3);
