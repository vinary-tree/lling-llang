...
///
/// This compiles `pattern` as a full-string regex and materializes the
/// reachable byte-token DFA states as a WFST. Since this API does not accept a
/// tokenizer, token IDs `0..=255` are interpreted as their corresponding
/// bytes; larger token IDs are not generated by the constraint.
pub fn from_regex<W: Semiring + From<f64>>(
    pattern: &str,
    vocab_size: usize,
) -> Option<WfstConstraint<W>> {
    let anchored_pattern = format!(r"\A(?:{})\z", pattern);
    ...
    let start = dfa
        ...
        .ok()?;
    let token_limit = vocab_size.min(usize::from(u8::MAX) + 1);
    ...
    state_map.insert(start, start_state);

    let mut queue = VecDeque::new();
    queue.push_back(start);

    while let Some(dfa_state) = queue.pop_front() {
        let from = state_map[&dfa_state];
        let eoi_state = dfa.next_eoi_state(dfa_state);
        if dfa.is_match_state(eoi_state) {
            fst.set_final(from, W::one());
        }

        for token in 0..token_limit {
            let byte = token as u8;
            let next = dfa.next_state(dfa_state, byte);
            if dfa.is_dead_state(next) {
                continue;
            }
            let to = if let Some(&state) = state_map.get(&next) {
                state
            } else {
                let state = fst.add_state();
                state_map.insert(next, state);
                queue.push_back(next);
                state
            };
            let token_id = TokenId::from(byte);
            fst.add_transition(WeightedTransition {
                from,
                input: Some(token_id),
                output: Some(token_id),
                to,
                weight: W::one(),
            });
        }
...
mod tests {
    ...
    fn advance_bytes<C: ConstrainedDecoder>(constraint: &C, input: &[u8]) -> DecoderState {
        let mut state = constraint.initial_state();
        for &byte in input {
            state = constraint
                .advance(&state, TokenId::from(byte))
                .expect("byte should be accepted by constraint");
        }
        state
    }

    #[test]
    fn test_from_regex_literal_sequence() {
        let constraint = from_regex::<TropicalWeight>("ab", 128).expect("valid regex");
        let initial = constraint.initial_state();
        let valid_initial = constraint.valid_tokens(&initial);
        assert!(valid_initial.is_valid(TokenId::from(b'a')));
        assert!(!valid_initial.is_valid(TokenId::from(b'b')));

        let after_a = constraint
            .advance(&initial, TokenId::from(b'a'))
            .expect("a should advance");
        let valid_after_a = constraint.valid_tokens(&after_a);
        assert!(valid_after_a.is_valid(TokenId::from(b'b')));
        assert!(!valid_after_a.is_valid(TokenId::from(b'a')));

        let after_ab = constraint
            .advance(&after_a, TokenId::from(b'b'))
            .expect("b should complete the match");
        assert!(constraint.is_accepting(&after_ab));
        assert_eq!(constraint.valid_tokens(&after_ab).count_valid(), 0);
    }

    #[test]
    fn test_from_regex_character_class_and_repetition() {
        let constraint = from_regex::<TropicalWeight>("[ab]+c", 128).expect("valid regex");
        let initial = constraint.initial_state();
        let valid_initial = constraint.valid_tokens(&initial);
        assert!(valid_initial.is_valid(TokenId::from(b'a')));
        assert!(valid_initial.is_valid(TokenId::from(b'b')));
        assert!(!valid_initial.is_valid(TokenId::from(b'c')));
        ...
        assert!(valid_after_a.is_valid(TokenId::from(b'a')));
        assert!(valid_after_a.is_valid(TokenId::from(b'b')));
        assert!(valid_after_a.is_valid(TokenId::from(b'c')));

        let after_ac = constraint
            .advance(&after_a, TokenId::from(b'c'))
            .expect("c should complete the match");
        assert!(constraint.is_accepting(&after_ac));
    }

    #[test]
    fn test_from_regex_full_match_only() {
        let constraint = from_regex::<TropicalWeight>("ab", 128).expect("valid regex");
        let initial = constraint.initial_state();
        assert!(constraint.advance(&initial, TokenId::from(b'b')).is_none());

        let after_ab = advance_bytes(&constraint, b"ab");
        assert!(constraint.is_accepting(&after_ab));
        assert!(constraint.advance(&after_ab, TokenId::from(b'c')).is_none());
    }

    #[test]
    fn test_from_regex_invalid_pattern() {
        assert!(from_regex::<TropicalWeight>("[", 128).is_none());
    }
