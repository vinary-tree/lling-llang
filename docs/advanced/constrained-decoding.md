...
computes a **token mask** `` `m` `` with `` `m[v] = 1 iff v ∈ allowed` `` and
overwrites the logits of disallowed tokens with `` `−∞` ``, so the sampled
...
| **CFG / CFL** | Context-Free Grammar / Language `` `G = (V, Σ, R, S)` ``. ([NOTATION](../NOTATION.md)) |
| **PDA** | Pushdown Automaton — finite automaton with a stack `` `Γ` ``; recognizes CFLs. ([NOTATION](../NOTATION.md)) |
| **FSM** | Finite-State Machine (no stack) — the compressed, stackless constraint. |
| `` `V` `` | The LLM vocabulary; a *token* is `` `v ∈ V` `` with id `` `TokenId = u32` ``. |
| `` `m` `` | Token mask: bit-vector with `` `m[v] = 1` `` iff token `` `v` `` is allowed. |
| `` `ℓ` `` | Logits — the LLM's per-token scores before masking. |
| `` `L` `` | The constraint language (the set of accepted token sequences). |
| `` `∣V∣` `` | Vocabulary size (cardinality); uses U+2223, not ASCII `|`. |
...
maintains a `DecoderState` `` `s = (q, γ)` `` — an automaton state `` `q` `` and
a PDA stack `` `γ` `` (`γ` is empty for a pure FSM). The **mask function** reads
the arcs leaving `` `q` ``:

```text
m[v] = 1   iff   ∃ an arc  q --v--> q'  in A          (v ∈ V)
m[v] = 0   otherwise
```

Masked decoding replaces the logit vector `` `ℓ` `` by `` `ℓ′` `` where
...
transition); `is_accepting(s)` tests whether `` `q ∈ F` ``.

| Component | Type | Role |
|---|---|---|
| `` `q` `` | `` `StateId` `` (`automaton_state`) | Current automaton state. |
| `` `γ` `` | `` `Vec<u32>` `` (`stack`) | PDA stack; empty for an FSM. |
...
automaton is `` `q0 --1--> q1 --2--> q2` `` with `` `q2` `` final. The mask is
...
|---|---|---|---|
| `WfstConstraint<W>` | no (FSM-like) | `WfstConstraint::new(automaton, ∣V∣)` | A constraint already expressed as a `VectorWfst`. |
| `CompressedFsmConstraint` | no | `CompressedFsmConstraint::from_wfst(&wfst, ∣V∣)` | Hot-path decoding — flat `(state, token) → state` map. |
...
Per step the cost is `` `O(∣arcs(q)∣)` `` to build the mask (or `` `O(1)` `` on
a cache hit) plus `` `O(∣V∣)` `` to apply it to the logits, i.e. `` `O(∣V∣)` ``
...
vocabulary; the decoder follows the green path, and at `` `q1` `` both `"k"` and
whitespace are legal, so `` `m[\"k\"] = m[ws] = 1` ``.
